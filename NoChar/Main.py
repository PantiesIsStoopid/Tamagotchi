...
# Function to update the GUI with the current metrics
def update_gui():
    metrics = get_metrics()
...
def decrease_metrics():
  ...
  if metrics:
    ...
    # Hunger conditions
    if new_hunger >= 100:
        messagebox.showinfo("Game Over", "Your Tamagotchi has died from overeating.")
        root.quit()
    elif new_hunger > 70:
        hunger_label.config(fg="red")  # Red for hunger above 70
    else:
        hunger_label.config(fg="black")  # Black for hunger between 0 and 70


    # Happiness conditions
    if new_happiness < 10:
      happiness_label.config(fg="red")
    if new_happiness == 0:
      messagebox.showinfo("Game Over", "Your Tamagotchi has died from sadness.")
      root.quit()

    # Energy conditions
    if new_energy < 30:
      energy_label.config(fg="red")
    if new_energy == 0:
      messagebox.showinfo("Game Over", "Your Tamagotchi has died from exhaustion.")
      root.quit()

    # Hygiene conditions
    if new_hygiene < 30:
      hygiene_label.config(fg="red")
      new_happiness -= 20

    # Bladder conditions
    if new_bladder > 90:
      bladder_label.config(fg="red")
    if new_bladder >= 100:
      messagebox.showinfo("Oops!", "Your Tamagotchi peed itself.")
      new_happiness -= 50
      new_hygiene -= 50
